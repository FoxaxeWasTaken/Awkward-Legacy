...
class TestImportExportRoundtrip:
    ...
    @pytest.fixture
    def galichet_gw_path(self):
        """Path to the test galichet.gw file."""
        # Try multiple possible paths for Docker environment
        possible_paths = [
            Path("/app/docs/parsing/galichet.gw"),
            Path(__file__).parent.parent.parent.parent.parent / "docs" / "parsing" / "galichet.gw",
            Path("docs/parsing/galichet.gw"),
        ]
    ...
    def test_roundtrip_galichet_file(self, test_db: Session, galichet_gw_path: Path):
        """Test complete round-trip with galichet.gw file."""
        # Step 1: Parse the original .gw file
        parser = GWParser(galichet_gw_path)
        ...
        print(f"Original: {original_families} families, {original_people} people, {original_notes} notes, {original_extended_pages} extended pages")
        ...
        try:
            ...
            print(f"Exported: {exported_families} families, {exported_people} people, {exported_notes} notes, {exported_extended_pages} extended pages")
            
            # Step 6: Verify data preservation
            # Families should be preserved
            assert exported_families == original_families, f"Family count mismatch: {exported_families} vs {original_families}"
            
            # People should be preserved (may be more due to children extraction)
            assert exported_people >= original_people, f"People count decreased: {exported_people} vs {original_people}"
            
            # Notes should be preserved
            assert exported_notes == original_notes, f"Notes count mismatch: {exported_notes} vs {original_notes}"
    ...
    def _verify_family_data_integrity(self, original: dict, exported: dict):
        ...
        for i, (orig_fam, exp_fam) in enumerate(zip(original_families, exported_families)):
            ...
            assert exp_children == orig_children, f"Family {i} children count mismatch: {exp_children} vs {orig_children}"
    ...
    def _verify_person_data_integrity(self, original: dict, exported: dict):
        ...
        for person in exported_people:
            ...
            assert "name" in person or "person" in person, f"Person missing name field: {person}"
    ...
    def test_gender_preservation(self, test_db: Session, galichet_gw_path: Path):
    ...
    def test_event_preservation(self, test_db: Session, galichet_gw_path: Path):
